...
//         position: 'relative'
//       }}>
//       <Link to="/" className="font-bold text-2xl text-white">Imagify</Link>
      
//       {user ? (
//         <div className="flex items-center gap-6">
//           <button className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800 border border-slate-700 text-white">
//             <span className="text-yellow-400">★</span>
//             <p className="text-sm font-medium">Credits Left: {user.credits ?? 'N/A'}</p>
//           </button>
//           <p className="pr-4 font-medium text-slate-400">Hi, {user.name ?? 'User'}</p>
//         </div>
//       ) : (
//         <div className="flex items-center gap-6">
//           <button 
    ...
    <nav 
      ...
      {user ? (
        <div className="flex items-center gap-6">
          ...
          {/* Profile image with hover logout, greeting is a button */}
          <button
            type="button"
            className="pr-2 font-medium text-slate-400 hover:text-slate-200 focus:outline-none focus:underline transition-colors"
            tabIndex={0}
            // Optionally, add onClick handler here if you want it to do something
          >
            Hi, {user.name ?? 'User'}
          </button>
